...
const CartItemCard = ({ product, isSearch, setSearch }) => {
  ...
  return (
    <div
      className={`m-auto flex flex-col gap-2  p-4 rounded-sm shadow-sm ${isSearch ? "bg-transparent text-white border-b border-gray-700/50" : "bg-white/[0.6]"} mb-2 max-w-xl ${isSearch ? "cursor-pointer hover:bg-white/5" : ""
        }`}
      onClick={() => {
        if (isSearch) {
          setSearch("");

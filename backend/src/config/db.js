...
// Force IPv4 first for DNS resolution to resolve Railway/cloud proxy IPv6 socket drops
...
const getDialectOptions = () => {
    const options = {
        ...
        family: 4 // Force IPv4 to prevent IPv6 socket drops on Railway proxies
    };
    if (shouldEnableSsl()) {
        options.ssl = {
            require: true,
